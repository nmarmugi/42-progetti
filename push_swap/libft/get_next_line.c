...
/*   Updated: 2024/03/07 12:42:09 by nmarmugi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*ft_up_buffer(char *buffer, int fd)
{
	...
	buffer_tmp = ft_calloc_gnl((BUFFER_SIZE + 1), sizeof(char));
	if (!buffer_tmp)
		return (NULL);
	read_char = BUFFER_SIZE;
	while (!ft_strchr_gnl(buffer_tmp, '\n') && read_char != 0)
	{
		...
		buffer = ft_strjoin_gnl(buffer, buffer_tmp, read_char);
...
static char	*ft_new_buffer(char *buffer)
{
	...
	if (buffer[i] != 0)
		...
		newline_buffer = 0;
	buffer_tmp = ft_strjoin_gnl(NULL, buffer + i + newline_buffer,
